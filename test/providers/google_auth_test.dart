...
import 'package:flutter_oauth/oauth/providers/google_auth.dart';
...
class MockGoogleSignInAccount extends Fake implements GoogleSignInAccount {
  ...
  Future<Map<String, String>> get authHeaders => Future.value(
      <String, String>{'Authorization': 'Bearer 123', 'X-Goog-AuthUser': '0'});
...
void main() {
  ...
  setUp(() {
    ...
    sut = GoogleAuthProvider(client: client, googleSignIn: googleSignIn);
  });
  ...
  group('GoogleAuthProvider.login', () {
    ...
    test(
        'should return errors when google people api fails to return user profile',
        () async {
      when(() => googleSignIn.signIn())
    ...
    test('should return user when google authentication is successful',
      ...
      when(() => client.get(any(), headers: any(named: 'headers'))).thenAnswer(
          (_) async => Response(json.encode(_googleProfile()), 200));
